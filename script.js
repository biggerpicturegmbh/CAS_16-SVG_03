...
const colorAchro = ["#e7eef1", "#1b1a16"];
...
function waveLines(wpTransX, wpOscil) {
  ...
  const wavePoints = Array(11)
    ...
    .map((d) => {
      if (countUp) {
        ++countW;
        if (countW >= 2) countUp = false;
      } else {
        --countW;
        if (countW <= 0) countUp = true;
      }
  ...
  const strD = `M0,${wpTransX} Q${wavePoints.map((d) => d.join(" "))}`;
